...
def build():
    ...
    layers = [
        (0.38, 1.45, 12.55, 1.15, GOLD, "第4章  时间窗", "只投射第3章 T1–T8。3年看透不变量能否被合同、接口和证据固定；5年看清谁拥有语义/QoD、近源运行时、动作证据和跨网结算控制点。不再另造趋势。"),
    ...
    thesis(s, "待验证的是共同外部行为，不是再加统一名称。更有把握冻结角色、稳定标识、最小元数据、交付外部行为，并与 QoD/授权/证据挂钩。Kafka、湖仓、完整 KG、独立 Fabric 或专用 Data Plane 都仍是未收敛实现选择。")
    add_table(
        s,
        Inches(0.38),
        Inches(1.38),
        ...
        [
            ...
            ["ITU-R IMT-2030  Recommendation", "六类场景、总体能力与2030目标；不定义网元、协议或数据生命周期", "形成AI原生、通感、孪生和泛在智能的数据需求来源", "不能推出QoD、Catalog、血缘或“6G Data Fabric”接口已成为IMT要求  [S1]"],
            ...
            ["O-RAN 开放接口 / ZSM 029", "O1/O2/A1/E2/R1；ZSM研究数据注册/发现、资产、认证、采集与编排", "按时标开放数据、策略、模型和动作；ZSM与目录/资产机制明显重合", "五类接口不是统一语义/QoD/血缘总线；工作项≠最终规范或独立Fabric  [S7][S43]"],
            ["TMF / GSMA / CAMARA", "自治运营、服务包装、开发者消费、用量、认证与结算", "覆盖数据/能力产品的外部行为和商业运营接口", "不能推出内部采集、QoD生成或无线动作路径已经统一  [S15][S16]"],
        ],
    ...
    add_table(
        ...
        [
            ...
            ["内部能力 ↔ CAMARA/Operate API", "内部网络能力经标准API向开发者暴露", "API一致不代表语义、QoD、责任、用途和计量一致", "内部契约到外部SLA的映射、跨网一致性测试和清结算字段"],
        ],
    ...
    add_table(
        ...
        [
            ...
            [
                ...
                "参考架构已成为统一SKU；DataOps/EIAP/NWDAF共享同一跨域语义、QoD和证据链；第三方Agent、异构湖仓和跨网API在相同契约下可双向迁移。",
    ...
    points = [
        (TEAL, "共同问题已经出现", "异构发现、近源处理、跨时标交付、语义/QoD、动作证据和外部运营均有真实需求，区别在实现责任与时标，不在名称。"),
        ...
        (LIME, "尚无端到端胜者", "没有中立证据证明任何一家已在多运营商、多厂商环境同时实现公共语义、通信QoD、开放后端和可信动作闭环。"),
    ]
    ...
    card(
        ...
        [
            ...
            "应让网络原生方案、外部编织方案和分时标混合方案，在同一组时延、可靠性、语义、QoD、证据、开放性与TCO指标下竞争。",
        ],
        NAVY,
        body_size=12,
    )
    ...
    add_table(
        ...
        [
            ...
            ["语义依赖：无线数据含义依赖对象、时空、配置、采样和测量误差；能访问≠适合当前任务", "模型和消费者只能重复解释、清洗与对齐；字段完整仍可能产生错误比较、训练偏差和闭环误判", "把数据封装为带消费者、Owner、契约和生命周期的产品，并以面向用途的QoD证明适用边界", "数据适用  ·  T1 可运营产品  ·  T6 QoD质量信封"],
            ...
            ["交易协同：行业应用需要跨运营商覆盖、一致行为、用途约束、责任界定、计量与清结算", "API数量增加但体验不一致、无人持续付费；内部成本、外部SLA、责任与收入无法闭环", "先形成内部可信产品，再经标准服务、跨网分发和行业结果逐层传递价值并完成结算", "价值可结算  ·  T7 四层商业链"],
        ],
    ...
    add_text(
        ...
        [
            ...
            "因果阅读：T2/T3 → T1/T6 → T4/T5 → T7。没有前一层，后一层不能靠产品命名越级成立。厂商只作外部校验，不替代第一性原理。",
        ],
        size=12,
        color=SLATE,
        spacing=1.12,
    ...
    card(
        ...
        [
            ...
            "领先信号：Schema/配置/QoD/权限变更自动生成影响清单并触发CI或策略检查。反证：目录仍靠人工扫描，事故早于目录发现变化。",
        ],
        TEAL,
        body_size=10,
    )
    card(
        ...
        [
            ...
            "全局：对象/语义、QoD模板、产品定义、策略、模型和版本发布。",
    ...
    add_table(
        ...
        [
            ...
            [
                "状态可知是T1/T6的前提：后续产品化才有稳定对象可引用，QoD才有来源和时效。",
                "T2：变更发现P95、影响覆盖、误报/漏报、对账差异、失效传播时长。T3：P99、跨域字节减量、断链可用、策略同步、版本回退。",
                "T1/T6只能做出静态目录，T4的动作会打到过期对象上。",
                "三家都有摄取/近源叙述；跨域事件对账和统一控制均未证明。OpenLineage证明事件可工程化，电信关键是网络对象/配置/QoD作用域  [S27]",
            ],
        ],
    ...
    # ------------------------------------------------------------------ 13 stage2
    s = new_slide(prs)
    topbar(s, "价值链 2  ·  数据适用", "T1 可运营数据产品  +  T6 面向特定用途的 QoD")
    thesis(s, "价值单位从「表、Topic、接口和PB」转为「持续满足明确消费者与任务的数据供给」。QoD是面向特定用途的适用性质量信封，不是覆盖所有数据的总分，也不是已冻结的统一3GPP指标。")
    card(
        s,
        Inches(0.38),
        Inches(1.40),
        ...
        [
            "最小完备集：明确消费者与任务、稳定标识和版本、Owner、Schema/语义、QoD、SLO、用途/权限、血缘、成本/计量、保留期及退出。只有目录条目、接口包装或Marketplace页面，仍不是数据产品。",
            ...
            "领先信号：产品契约进入版本基线，生产者和消费者共同签署SLO/QoD。反证：每个场景仍重新采集清洗，产品长期无人消费，Owner与退出机制缺失。",
        ],
        TEAL,
        body_size=10,
    )
    card(
        ...
        Inches(6.15),
        Inches(3.55),
        "T6  QoD成为模型与闭环的准入标准  ·  中高确定性",
        [
            "定义：用一组可测试的质量信封说明，数据在何种对象、时空、配置与采样条件下，可以被哪个模型或闭环安全使用。同一KPI在不同小区/频段/波束/配置/采样窗口下不可直接比较。",
            ...
            "领先信号：QoD进入数据契约、模型卡、跨域接口和动作放行条件。反证：仍只统计记录量、接口成功率、完整性，或用一个总分掩盖差异。",
    ...
    add_table(
        ...
        [
            ...
            [
                ...
                "T4/T5会把不合格数据送进闭环，T7的外部服务会卖出不可负责的结果。",
    ...
    card(
        ...
        [
            "四段：Fabric提供版本化产品、上下文、QoD与策略 → 模型/Agent依据意图选择数据、工具和候选动作 → 执行面按身份、作用域、租约和风险预算动作 → 动作、结果、回滚与漂移写回共同证据图。",
    ...
    card(
        ...
        [
            "关系：Data Fabric向NDT提供带时间、配置、QoD与血缘的版本化状态快照；NDT对候选动作做反事实预演并输出收益、冲突和副作用；策略引擎据此放行、灰度或拒绝；生产结果再回流校准。NDT是证据链中的消费者和生产者，不是Fabric同义词。",
    ...
    add_table(
        ...
        [
            ["关口检验", "验证指标", "若不成立", "2.3外部校验"],
            [
                "行动可信是T7的前提：才有资格把闭环结果卖给外部；标准才有动作证据可测。",
    ...
    # ------------------------------------------------------------------ 15 stage4
    s = new_slide(prs)
    topbar(s, "价值链 4  ·  价值可结算  ·  T8 横切", "T7 四层商业链成立的前提，是前三关口已经可验证")
    thesis(s, "之所以称为商业链，不是因为多了API，而是每层都有不同生产者、购买者、成本、责任与结算对象。T8：T1–T7的需求会留下，承载它们的拓扑、组件边界和名称未必统一。")
    layers7 = [
        ("① 内部可信生产", "供给：带语义、QoD、用途和成本的数据/分析产品。买方：网络产品线、运维、AI和API团队。价值：复用、降本、缩短交付。"),
        ("② 标准服务包装", "供给：QoD、位置、身份、状态或分析服务。买方：行业平台、应用与CPaaS。价值：SLA、调用/会话收费。"),
        ("③ 跨运营商分发", "供给：一致目录、认证、覆盖、用量和清结算。买方：需要全国/多国覆盖的开发者。价值：渠道佣金、收入分成。"),
        ("④ 行业结果结算", "供给：交易成功、体验改善、风控或运营结果。买方：金融、车联、工业、媒体。价值：按结果或业务量定价。"),
    ]
    for i, (title, body) in enumerate(layers7):
        card(s, Inches(0.38 + i * 3.2), Inches(1.40), Inches(3.05), Inches(1.72), title, [body], TEAL if i < 2 else AMBER, title_size=12, body_size=10)
    card(
        s,
        Inches(0.38),
        Inches(3.22),
        ...
        "T7成立条件  ·  中等确定性",
        [
            "外部服务的一致性上限由内部数据产品稳定性决定；单运营商覆盖通常不足以支持规模行业应用；标准API会降低接口稀缺性，价值上移到交易/会话/结果。",
            "先产品化内部能力，再映射CAMARA或行业API；不能用API数量倒推内部成熟度。原始感知数据出售不是默认模式。",
            "领先信号：内部产品契约与外部SLA可追溯，Operate API、聚合渠道与用量结算联通。反证：API数量增长但无跨网一致、持续消费者、责任边界和收入。",
        ],
        TEAL,
        body_size=10,
    )
    ...
    add_text(
        ...
        [
            ...
            "趋势边界：事件化≠全量流事件化；产品化≠上架目录；协同闭环≠Agent接管快环；NDT≠安全证明；QoD≠单一总分；商业链≠API数量；能力集合≠标准已放弃新NF。越过这些边界，趋势判断就会退化为营销术语。",
    ...
    thesis(s, "「看透」是把趋势变成可验证、可证伪、可停止的工程命题；「看清」是观察谁掌握语义/QoD、近源运行时、Agent/NDT证据、跨网结算和互操作规则。不猜网元名称，不再另造趋势。")
    add_table(
        s,
        Inches(0.38),
        Inches(1.36),
        ...
        [
            ...
            ["T1 数据产品  高", "故障/节能/移动性/QoE或模型特征中，至少少量产品具有消费者、Owner、Schema、QoD、SLO、用途与退出；同一产品跨第二场景/后端复用", "跨域供给从项目接口转为版本化产品组合；高价值产品进入主航道版本和运营体系，部分按任务或结果计量", "若仍按项目重复取数清洗，“产品化”只是目录改名；原始数据出售也未必成为主模式"],
            ["T2 元数据事件  高", "Schema、配置、拓扑、QoD、权限和血缘变化能触发影响分析、契约测试、缓存/模型失效或审批；同时保留基线快照和周期对账", "元数据事件成为质量治理、模型重验和策略执行的共同状态输入；跨域对象ID、事件模型、血缘与影响图形成控制点", "若事故仍早于目录发现变化，控制面仍是被动盘点；事件不应成为快环在线依赖"],
            ...
            ["T6 QoD准入  中高", "按数据族形成质量信封并进入数据契约、模型卡或接口；至少覆盖采样代表性、时空覆盖、同步/配置一致、测量置信及训练—服务偏差", "不同数据族的质量信封收敛公共属性并进入互操作测试；QoD成为模型、闭环和跨域产品的共同验收语言", "若仍只报完整性和接口成功率，适用性尚未被管理；短期难出现覆盖所有数据和任务的统一总分"],
            ["T7 商业链  中", "内部产品契约能映射到至少一个标准服务；跨网行为、用量和责任可计量；出现持续付费消费者或明确业务Owner，而非一次性展示", "内部生产、标准服务、跨网聚合和行业结果之间分工清晰；部分场景从调用定价走向会话、交易或结果定价", "若API增长而无消费者、收入和责任边界，商业链尚未成立；ISAC/数据空间付费规模仍取决于需求、隐私和制度"],
            ["T8 能力集合  低—中", "Rel-21讨论开始收敛发现、处理、存储、暴露、QoD和生命周期等能力及接口；产品原型证明可插拔后端与多种部署映射", "标准优先冻结能力语义、信息模型、接口行为和一致性测试；增强既有功能、边缘Agent、SMO服务或新NF可按部署组合", "若3GPP明确冻结独立统一数据平面及必选NF，则需调整；在此之前不押注名称，也不预设标准一定拒绝新功能"],
        ],
    ...
    add_text(
        ...
        Inches(1.15),
        [
            "1–3年决策：先做可证伪底座。优先建立T2/T3事件和两级运行，再用T1/T6证明可复用、可验收；T4/T5从只读、建议、影子运行开始；T7只认真实付费、跨网一致和履约成本；T8以进入工作文本、参考实现和互操作测试为标准验证。",
            "3–5年决策：看谁拥有控制点。设备商的稳定优势在网络语义、QoD和近源执行；云/数据平台在模型、Catalog与开发工具；聚合商在分发与结算。运营商将要求对象、策略、证据可导出，运行时和后端可替换。",
            "收束：3年让每条趋势都有可验证、可停止的证据；5年看清谁拥有语义/QoD、近源运行、动作证据、标准接口与跨网结算。投射基于截至2026-08的公开材料，不等于3GPP已采纳，也不构成厂商产品承诺。",
        ],
        size=10,
        color=NAVY,
        spacing=1.06,
