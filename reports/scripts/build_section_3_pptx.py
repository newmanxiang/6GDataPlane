...
def slide_p5(prs) -> None:
    ...
    add_text(
        ...
        12.27,
        0.44,
        ["内部语义、任务适用性、权限和血缘不稳，API只会把差异暴露给客户。单运营商覆盖通常不足以支撑规模行业应用，必须经聚合层统一发现、认证、行为、用量和清结算。跨组织使用还需持续执行用途、撤销、责任和收入分配。"],
        size=9.5,
        color=SLATE,
        spacing=1.03,
    )
    ...
    a_rows = [
        ("供给", "Quality on Demand（QoD）、位置、身份、状态、分析服务；标准化网络能力API；最小必要、可撤销的数据产品。"),
    ...
    evidence_bar(
        ...
        [
            ...
            ("证据状态", "路径A：CAMARA/Open Gateway、TM Forum Operate API、聚合渠道和中国移动Quality on Demand（QoD）商用已有信号。路径B：规模付费、结果责任和归因机制公开证据不足。", NAVY),
