...
def slide_p1(prs) -> None:
    ...
    constraints = [
        ...
        ("约束 2  ·  复用", "语义要跨域", "模型与产品要跨RAN、Core、OSS/BSS复用，必须有稳定对象、上下文、任务适用性和版本。名称相似不等于跨域语义已经对齐。"),
    ...
    pills = [
        ...
        ("任务适用性/ 产品契约", "用途适用性、Owner、SLO、计量与退出"),
...
def slide_p2(prs) -> None:
    ...
    card(
        ...
        "较可能先冻结",
        [
            "角色、稳定标识、最小元数据、交付控制、任务适用性/授权/证据挂钩点，以及一致性测试。",
...
def slide_p3(prs, fig18: Path, north: Path) -> None:
    ...
    card(
        ...
        [
            ...
            "代价是跨产品Schema、任务适用性、策略和版本可能长期碎片化；协调成本若高于替换成本，联邦优势会下降。",
        ],
        AMBER,
        fill=SOFT_AMBER,
    )
...
def slide_p4(prs, huawei: Path) -> None:
    ...
    thesis(
        s,
        "这是一条“先重构执行、再决定治理边界”的路线：DO编排拓扑，DA近源处理，DCP异步分发。它首先是6G数据面机制，能吸收部分发现、编排和交付能力，但不会自动解决跨域语义、任务适用性、血缘与产品责任。不能把“补成Fabric”当成唯一终点。",
    )
...
def slide_p6(prs) -> None:
    ...
    add_table(
        ...
        [
            ...
            ["结构性代价", "跨产品语义、任务适用性、策略和版本协调持续存在", "新增顺序、持久化、容灾、移动性与可移植性", "多层SKU可能只在品牌层组合并形成软锁定"],
    ...
    controls = [
        ...
        ("2 任务适用性与产品契约", "定义数据对特定任务是否适用、谁负责、满足何种SLO、如何计量、何时降级或退出。没有契约，跨域交付只能停留在项目集成。"),
    ...
    add_text(
        ...
        12.23,
        0.40,
        ["真正的平台地位不是把所有数据收到一个平台，也不是把Data Mesh写成自智内核，而是让消费者遵循稳定语义、任务适用性和证据契约，同时让状态—意图—策略—动作—结果—回滚闭环可验证，并证明运行时、后端和渠道仍可替换。"],
        size=10.5,
        color=SLATE,
        spacing=1.03,
    )
