...
def build():
    ...
    trends = [
        ...
        ("T7 · A中高 / B低—中", "商业化沿责任边界分化：能力服务卖输入，结果服务承诺输出",
         "买方保留决策与动作权时购买可组合能力；供应方承担决策、动作和效果责任时形成结果型托管服务。",
         "落地：A按调用/会话/订阅跨网扩张；B先定义结果基线、归因、授权、风险与回滚，再按托管费、增益或结果分成。",
         "反证：A只有API数量无持续收入；B无法定义基线、归因、定责或回退；两条路径混算收入与风险。"),
