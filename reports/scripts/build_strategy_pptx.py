...
def build():
    ...
    needs = [
        ...
        ("需求3", "看清标准与竞争窗口", "区分既定积木、Rel-20研究、Rel-21规范、厂商主张与商用产品，避免误判与错过语义/QoD话语权。"),
    ]
    ...
    thesis_box(s, "Data Fabric可做6G数据管理层的设计骨架（尤其Non-RT/跨域供给），但不能原样进入通信网：缺RAN语义、无线QoD、时标隔离、动作安全与多Agent治理；绝不能成为毫秒快环的远端在线依赖。", height=Inches(0.78))
    ...
    card(s, Inches(6.75), Inches(1.9), Inches(6.1), Inches(1.55), [
        ...
        ("补齐 RAN 对象与时空语义、无线 QoD、亚秒时标隔离、动作安全、NDT 证据门槛、多 Agent 冲突治理；不替代 RAN 协议 / SA2·SA5 / O-RAN。", 11, False, INK, 0),
    ...
    trends = [
        ("T1 · 高确定性", "网络数据从采集对象变为可运营产品",
         ...
         "落地：先做移动性/节能/故障/QoE/模型训练高复用产品；必须带对象范围、事件时间、语义版本、QoD、用途、Owner。",
    ...
    s = new_slide(prs)
    bg(s)
    topbar(s, "03  TRENDS T5–T8", "后半因果链：证据闸门 → QoD标尺 → 商业分层 → 能力集合化")
    thesis_box(s, "T5–T8回答「如何证明、计量与标准化」：高风险动作要孪生证据，无线数据要通信级QoD，商业走内部产品→API→渠道分层，标准更可能冻结能力集合而非固定新网元。")
    trends = [
        ...
        ("T6 · 中高确定性", "QoD成为6G数据标准与产品竞争核心标尺",
         "可用性由网络状态、采样与误差决定，不能只用完整性与新鲜度。",
         "落地：按数据族建质量信封；纳入采样覆盖、时空代表性、同步、配置、标签来源、训练—服务偏差；绑定产品SLO与动作风险。",
    ...
    card(s, Inches(0.45), Inches(1.85), Inches(6.15), Inches(4.8), [
        ...
        ("工程底座", 11, True, TEAL, 2),
        ("元数据事件、跨O1/E2/A1/R1映射、RAN QoD、数据契约、模型/动作血缘成领先试点基线。", 10, False, MUTED, 5),
        ("产品形态", 11, True, TEAL, 2),
    ...
    card(s, Inches(6.8), Inches(1.85), Inches(6.05), Inches(4.8), [
        ...
        ("胜负手", 11, True, CYAN, 2),
        ("多厂商语义映射、QoD标准、Agent可信运行时、NDT证据门槛、跨域产品运营形成长期壁垒。", 10, False, MIST, 0),
    ], fill=NAVY)
    ...
    thesis_box(s, "正确战略不是再造对标 IBM/Informatica/Nokia Data Suite 的通用编织平台，而是把 AIR Net 数据引擎升级为主航道共用的横向数据控制能力；争夺「语义+QoD+近源执行+有界供数」，而非「大一统数据面」品牌。", height=Inches(0.8))
    anchors = [
    ...
    evid = [
        ("E4 API", "中国移动QoD：CAMARA+中兴NEF；25家客户、月调用>3400万、收入证据"),
        ...
        ("E0 假设", "RAN QoD/近源Agent/跨接口事件 → 待PoC与付费验证"),
    ]
    ...
    ctrls = [
        ("C1 语义与 QoD", "对象映射·质量信封·配置/时空/采样", "中兴必须主导", TEAL),
    ...
    thesis_box(s, "用 AIR Net 数据引擎抢占「电信语义—QoD—近源执行—有界供数」四位一体控制点；以 ZSM 029 / SA2·SA5 把接口与语义写成标准事实，以场景闭环证明价值——而不是先宣称拥有完整 6G Data Fabric。")
    scores = [
    ...
    thesis_box(s, "中兴最优解是「数据引擎外溢 + 标准卡位 + RAN近源QoD + 动作/主权双证据」；用跨场景复用、闭环收益、可回滚性与策略持续强制证明「可信供数」，而不是用功能清单证明「拥有Fabric」。")
    ops = [
        ("O2 首选", "数据引擎外溢为能力组件",
    ...
    card(s, Inches(0.45), Inches(1.85), Inches(4.0), Inches(3.5), [
        ...
        ("语义、QoD、契约、工具接口与标准参考实现由同一核心团队负责；标准文稿必须来自可运行代码，核心代码必须对齐标准对象。", 11, False, MIST, 0),
    ], fill=NAVY)
    ...
    thesis_box(s, "策略总断言：先把数据引擎做成主航道共用的「可信供数控制面」——用标准写下语义与接口，用近源能力守住RAN QoD，用动作证据与主权策略约束自治和跨组织流通；其余一律后置。【D】条线可推，【R】需公司决策。")
    strategies = [
        ...
        ("6【R】", "跨线虚拟团队", "统一对象/QoD/北向；禁重复造目录"),
        ("7【D】", "NDT证据链", "快照/误差/灰度/回滚成放行条件"),
        ("8【R】", "运营商试点与路标", "独立ROI；承载映射与API写入路标"),
        ("9【D+R】", "Build/Partner/Buy", "自研语义QoD近源证据；合作湖仓模型"),
    ...
    write_box(bpb, [("Build：语义/QoD/近源Agent/产品契约与动作证据　｜　Partner：湖仓Catalog流批/模型/隐私计算　｜　Buy：连接器扫描可观察工具", 10, False, NAVY)])
    ...
    add_table(
        ...
        [
            ...
            ["AP2", "ZSM029+SA2/SA5标准包", "DMA参考实现；语义/QoD/双栈提案", "联署+有效输入+产品同步", "P0"],
            ["AP3", "语义包与QoD信封v1", "核心对象+质量规则+评测集", "跨两域；准确率/覆盖门槛", "P0"],
            ...
            ["AP7", "RAN近源Agent时间盒", "流内标注/QoD/断链/异步证据", "资源P99达标否则停止", "P1"],
    ...
    add_table(
        ...
        [
            ...
            ["ZSM029+SA2/SA5建立语义/QoD/接口/测试话语权", "不押注独立数据面名称；不以提案数代替采纳", "Rel-20/21最终能力包与运营商联署"],
            ["O1/O7/O8验证近源、可信自治与主权强制并设停止线", "Fabric不进快环同步单点；Agent无证据不改网", "站点开销、QoD增益、NDT误差、策略一致率"],
